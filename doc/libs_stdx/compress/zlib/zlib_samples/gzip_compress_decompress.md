...
func compressFile(srcFile: String, destFile: String): Int64 {
    ...
    try (input: File = File(srcFile, Read), output: File = File(destFile, Write)) {
        // 创建压缩输出流，指定 Gzip 格式
        var compressStream = CompressOutputStream(output, wrap: GzipFormat, bufLen: 10000)
...
func decompressFile(srcFile: String, destFile: String): Int64 {
    ...
    try (input: File = File(srcFile, Read), output: File = File(destFile, Write)) {
        // 创建解压输入流，指定 Gzip 格式
        var decompressStream = DecompressInputStream(input, wrap: GzipFormat, bufLen: 10000)
