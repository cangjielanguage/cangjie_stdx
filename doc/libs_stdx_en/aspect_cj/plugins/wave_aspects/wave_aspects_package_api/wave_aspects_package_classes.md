...
> **Note:**
>
> `WaveAspects` is auto-registered via the `@CHIRPlugin` macro; no manual call to `PluginManager.registerCHIRPlugin` is needed. When `executeCHIRPlugins` is invoked, an instance is created and `run` is called automatically.
...
<!-- verify -->
```cangjie
import stdx.aspect_cj.plugins.wave_aspects.*
